# install.md — instalar y correr `agent-governance-checks`
...
> 1. **Instalación** — qué necesitas y cómo se corre.
...
> **El kit NO se instala con `pip install .` — se corre POR RUTA** (H2, v1.0). Su
...
> ver §1.3-§1.5). Instala **solo la dependencia** (`pip install pyyaml`) y corre por ruta.
...
### 1.3 Correr el gate técnico (criterio A)
...
(§3) se mantiene: M2 no ejecuta el motor; el orquestador corre los dos métodos y contrasta.

El modo `ambas` corre además la **pasada audit** (`correr_audit`): verifica el audit trail
...
```bash
# Corre TODOS los detectores sobre tu corpus (no sobre las fixtures del kit).
...
mismo que `Runner.correr(root=..., config=...)`, así que no tienes que escribirte un
runner a mano. `--root` y `--fixtures` son excluyentes: con `--root` se corre TU corpus;
sin él, el gate corre sobre las fixtures del kit (comportamiento por defecto, §1.3).

**Empieza siempre por copiar tus fixtures a `gate/fixtures/` y correr el gate** (o apunta
...
> aborta nombrándola — **nunca** aplica un default silencioso. Un default reintroduciría
...
- **Convención de nombres de skill (elección deliberada, no dogma).** El `entry_regex`
  ...
  **minúsculas-kebab** (`orquestador-maestro`, `contable-experto`). Es una **convención
  deliberada** del corpus TechAcces —verificada: las 40+ skills reales la cumplen sin
...
El gate técnico (criterio A) prueba **fontanería**: que los detectores corren y detectan
...
4. Corre los cinco detectores (`python gate/run_gate.py`; el quinto, `audit_chain`,
